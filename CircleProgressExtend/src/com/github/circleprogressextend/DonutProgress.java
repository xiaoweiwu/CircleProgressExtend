...
public class DonutProgress extends View {
    ...
    protected void initByAttributes(TypedArray attributes) {
        ...
        suffixText = TextUtils.isEmpty(attributes.getString(R.styleable.DonutProgress_donut_suffix_text)) ? default_suffix_text : attributes.getString(R.styleable.DonutProgress_donut_suffix_text);
    ...
    private float getProgressAngle() {
        return getCurrentProgress() / (float) max * 360f;
    }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        String text = this.text != null ? this.text : prefixText + currentProgress;
        if (!TextUtils.isEmpty(text)) {
            float textHeight = textPaint.descent() + textPaint.ascent();
            canvas.drawText(text, (getWidth() - textPaint.measureText(text)) / 2.0f, (getWidth() - textHeight) / 2.0f, textPaint);
            float textWidth = textPaint.measureText(text);
            textPaint.setTextSize(suffixTextSize);
            canvas.drawText(suffixText, getWidth() / 2.0f  + textWidth/2.0f + suffixTextPadding, (getWidth() - textHeight) / 2.0f, textPaint);
        }
