...
public class CircleProgress extends View {
	...
	public void setProgress(int progress) {
		...
		animator.setIntValues(currentProgress,progress);
	...
	public String getDrawText() {
		return getPrefixText() + getCurrentProgress() + getSuffixText();
	}
	...
	protected void onDraw(final Canvas canvas) {
		...
		scheduleDraw(canvas);
		// float yHeight = getCurrentProgress()/ (float) getMax() * getHeight();
		// float radius = getWidth() / 2f;
		// float angle = (float) (Math.acos((radius - yHeight) / radius) * 180 /
		// Math.PI);
	...
	void scheduleDraw(Canvas canvas) {
		float yHeight = currentProgress / (float) getMax() * getHeight();
		float radius = getWidth() / 2f;
