package com.github.circleprogressextend;
...
public class ArcProgress extends View {
    ...
    protected void initByAttributes(TypedArray attributes) {
        ...
        innerBottomTextSize = attributes.getDimension(R.styleable.ArcProgress_arc_inner_bottom_text_size, default_inner_bottom_text_size);
        innerBottomTextColor = attributes.getColor(R.styleable.ArcProgress_arc_inner_bottom_text_color, default_inner_bottom_text_color);
        innerBottomText = attributes.getString(R.styleable.ArcProgress_arc_inner_bottom_text);
    }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        String text = String.valueOf(getCurrentProgress());
        ...
        if (!TextUtils.isEmpty(text)) {
            ...
            canvas.drawText(suffixText, getWidth() / 2.0f  + textWidth/2.0f + suffixTextPadding, textBaseline, textPaint);
        }
        float bottomTextBaseline=0.0f;
        float bottomTextHeight=0.0f;
        if (!TextUtils.isEmpty(getBottomText())) {
            textPaint.setTextSize(bottomTextSize);
            bottomTextHeight = textPaint.descent() + textPaint.ascent();
        ...
        if (!TextUtils.isEmpty(getInnerBottomText())) {
        	innerBottomTextPaint.setTextSize(innerBottomTextSize);
        	float bottomTextTopLine = bottomTextHeight+bottomTextBaseline;
        	float innerBottomTextHeight = innerBottomTextPaint.descent() + innerBottomTextPaint.ascent();
        	float innerbottomTextBaseline = bottomTextTopLine+(textBaseline-bottomTextTopLine-innerBottomTextHeight)/2.0f;
//            float innerbottomTextBaseline = textBaseline+textHeight  - (innerBottomTextPaint.descent() + innerBottomTextPaint.ascent()) / 2;
            canvas.drawText(getInnerBottomText(), (getWidth() - innerBottomTextPaint.measureText(getInnerBottomText())) / 2.0f, innerbottomTextBaseline, innerBottomTextPaint);
        }
        incrementProgress();
    }
